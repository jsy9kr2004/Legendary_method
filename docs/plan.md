...
- [ ] **dashboard_worker 14건 tz 실패 (사전 존재, 2026-05-24 발견)** — `tests/test_dashboard_worker.py` 14건이 `src/scalping/exit/triggers.py:302` `now - holding.entry_time` 에서 `can't subtract offset-naive and offset-aware datetimes` 로 실패. 이번 작업(결정 레포트) 과 무관한 별도 버그 — `git stash` 로 내 변경 제거 후에도 동일 실패 확인. Holding.entry_time naive ↔ now KST aware 정합 필요. 별도 round 에서 수정.
