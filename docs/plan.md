...
- [x] **단저단고 재설계 + 라이브 dry-run 통합 (2026-05-27/28, 본 세션)** ★ — `docs/scalping-redesign-2026-05-27.md` + 모듈 구현. (1) 패러다임 전환: 모멘텀/돌파(Buy.Score) → 분봉 단저단고. (2) v1~v3.2 백테스트 + AUC feature selection → gross 양수 도달 단 net 음수, vol_spike/bid_ask_ratio/vol_accel 다 noise (AUC 0.50~0.52), 종목별 패러다임 반대 (한온 평균회귀 / 삼성전자 추세). (3) **봉 close 지연 +0.37% 슬리피지 = gross 음수 주범 폭로** — 보정 시 모든 버전 net 양수 도달. (4) 모듈: `src/scalping/bars.py`, `src/scalping/signals/mean_reversion.py`, `src/common/universe.py`, `src/research/backtest_mean_reversion.py`, `src/analysis/mr_alignment.py`. (5) 라이브 dry-run 통합 완료: `MONITOR_MEAN_REVERSION=1` 카드 표시, `MONITOR_MR_UNIVERSE=1` universe 게이트, tick_log 에 mr_sigB/mr_sigS/mr_reason 컬럼 영구 로깅, 매매일지 자동 정합도 통계 (`python -m src.analysis.mr_alignment YYYY-MM-DD`). (6) 라이브 매매 영향 X — 사용자 카드 보고 직접 매매, 검증 후 운영 전환. 위험 관리: 지정가 매수 + 유동 큰 종목. 종배 그대로. 5/25 [[scalping-method-separation]] 결론 강화 + 확장.
- [x] **Eod.Pick 하한 0 초과 + 상한 NXT 조건부 (round 42, 2026-05-25)** — 레전드 종배 픽(image/jb.png) vs 시스템 후보 정합 분석(`scripts/analyze_legend_picks.py`: 50%만 겹침, 탈락 픽이 오히려 더 갭상) + 1년 top50 backtest(`scripts/backtest_eod_floor.py`: 0~5% 버킷 56%/+0.41% 양의 EV). `MIN_DAILY_RETURN` 5→0 (ret>0 초과, 보합/하락 제외 — 단타 LEADER_MIN_DAILY_RETURN_PCT 와 일관). 상한은 `extract_candidates(nxt_set=...)` 로 행별 effective_max: NXT 가능/추정 29.5% / 불가 27% (27% 단일컷이 NXT 표시보다 먼저 상한가를 제외하던 모순 해소). pipeline·scheduler nxt_set 주입, decision 라벨/푸터 갱신. test +5, 전체 pass. demo dry-run 통과. memory: [[project-legend-picks-mismatch]].
- [x] **단저단고 v4 — 강망치 단저 전환 + 단고/per-stock 폐기 (2026-05-30, image/3.PNG)** ★ — 사용자 토론으로 시그널 전면 재설계 + 코드/문서 반영 완료. (a) 통설 검증 ✓ (망치형·trailing·과매도 통과 / 고정익절·피보·정교한 단고 배제, ritual a). (b) train(5/18~22)/OOS(5/27~29) 분리 백테스트 ✓ (`src/research/backtest_zigzag_compare.py`): **진입=swing저점∩강망치(아래꼬리≥0.6), 청산=trailing −1.0%** → OOS 왕복 **+0.131%**(41건) vs 기존 oversold STRONG→overbought STRONG **−1.390%**(1건=미작동). **단고(매도) STRONG 폐기**(청산 alpha 죽임 OOS −0.972%, 매도 후 68% 재폭등), **per-stock weight 제거**(글로벌 룰, 강망치 발화 드물어 표본 X). 코드: `classify_zigzag()`/`_analyze_zigzag()` — `analyze_minute_bars` 무조건 강망치 (재시작 즉시 적용, oversold/단고/per-stock 완전 폐기, 롤백은 git revert), 상수 `HAMMER_LOWER_WICK=0.6`/`MR_TRAIL_PCT=1.0`. 문서: CLAUDE.md 단저단고 룰 + `docs/scalping-redesign-2026-05-27.md` §v4 + `data/journal/2026-05-29.md`. **자동매매 X, 카드 STRONG 단저 매수신호 표시 + 사용자 직접 매매.** −2% 손절 검증: 강망치엔 −1% 즉시 trailing 우월(사용자 룰 −2%까지 버티는 armed 는 OOS 음수, 진입 실패 빨리 컷이 유리), trail1.0 유지. 남은 것: dry-run 1~2주 누적 → trailing 폭/보조 가점(과매도·저거래량)/운전수 + 사용자 실매매 −2% 심리 재검토 (OOS 41건·3일 표본 한계).
- [ ] **단저단고 v4 dry-run 누적 후 재검증 (2026-05-30~)** — 라이브 1~2주 누적(mr_alignment) 후 검증할 후보. 표본 OOS 41건·3일 한계라 지금 확정 X. **검증 후보 (우선순위순)**: (1) **시간 청산 30분(10봉)+trailing** — 청산 보완 중 유일하게 OOS 약개선(+0.181% vs 현재 +0.131%)이나 train 역전 + 차이 0.05%p라 노이즈 의심, dry-run 1순위. (2) **진입/종목 선별** — 매도 alpha 본질적으로 작으니 개선 여지는 강망치 변형·강세 추세 종목 선별에 더 큼. (3) trailing 폭(현재 −1.0%) 변동성별 조정. (4) 보조 가점(과매도·저거래량 — 강망치 내부 +0.4~1.5%였으나 표본 9~29건). (5) 운전수 per-stock(발화 누적 후). (6) 사용자 실매매 −2% 손절 심리 vs −1% trailing. **★ 배제됨(재시도 X, ritual 통설+OOS 둘 다 기각)**: ATR/Chandelier trailing(통설 "percentage > Chandelier" + OOS −0.02~−0.64%), 부분 익절(OOS −0.05~−0.13% 과적합), 고정 익절 target(OOS 음수), 정교한 단고 매도(alpha 죽임), −2%까지 버티는 armed 손절(OOS 음수). 상세 `data/journal/2026-05-29.md` + `docs/scalping-redesign-2026-05-27.md` §v4.
- [ ] **NXT 매매가능 목록 크롤러 (round 42 후속)** — 현재 `data/meta/nxt_tradable.txt` 미적재라 전 종목 '추정 가능'으로 상한 29.5% 일괄 적용. nextrade.co.kr 단계별 매매가능 종목 크롤러 + 월 1회 갱신 (테마/WICS 동일 운영) 적재 시 NXT 가능/불가 정밀 구분. KIS NXT 애프터마켓 시세/주문 파라미터(시장구분 NX/통합)도 레포 샘플 확인 후 v1.
- [ ] **종배 하한 0 / 상한 29.5 운영 1일 검증 (round 42)** — 2026-05-26 14:50 결정 레포트부터 적용. dry-run(demo) 통과했으나 실데이터 universe 에서 후보 폭/품질 1일 확인 후 확정. 특히 NXT 미적재 상태에서 상한 29.5 전역 적용이 점상한가 noise 를 끌어오는지 점검.
...
- [x] **tick_log intraday_high 컬럼 100% 0 결함 (2026-05-21)** — KIS volume-rank API `stck_hgpr` 가 0 으로 응답되는 알려진 결함 (`scheduler.py:626-651` 코멘트). 운영 grader R14k 계산은 `worker.py:773-781` 의 bars 최고가 fallback 으로 정확. 단 tick_log 컬럼 자체가 100% 0 으로 저장돼 향후 backtest 도구가 이 컬럼 쓸 경우 위험. 옵션 A 적용: `build_tick_log_row` 에 `intraday_high_override` 인자 추가 (`src/data/tick_log.py`), worker 의 monitored / non-monitored 두 호출 위치에서 fallback 적용 후 값 전달 (`src/dashboard/worker.py:881, 951`). 회귀 테스트 4 신규 (`test_tick_log.py`) + 기존 16/26/grader 71 통과. 매매일지 `data/journal/2026-05-21.md` §사용자 답변 #5 참조.
- [x] **외인/기관/프로그램 데이터 fetch endpoint 교체 (2026-05-21)** — 사용자 보고: "종배 레포트와 대시보드 외인/기관/프로그램 다 0". 진단: 기존 `inquire-investor` (FHKST01010900) 가 (a) 외인/기관 필드 빈 문자열 응답 (5/21 운영 로그에서 78,341 회 warning) + (b) 프로그램 필드 자체 미제공 (4일 연속 100% 0). KIS GitHub `open-trading-api` 확인 후 두 종목별 endpoint 도입: `investor-trend-estimate` (HHPTJ04160200) + `program-trade-by-stock` (FHPPG04650101). MTS 투자자동향/프로그램 화면 동일 데이터. 외인/기관 갱신 4~5회/일 (09:30~14:30). `fetch_investor_flow` 본문만 교체, 반환 dict 키 유지 → caller 변경 0. 외인/기관 `_value` 는 수량 × program 응답의 현재가 추정. 신규 함수 2 + 통합 테스트 13. 946 pass. 진단 스크립트 `scripts/diag_investor_trend.py`.
- [ ] **KIS volume-rank stck_hgpr 결함 근본 해결** — 위 옵션 A 는 회피책. 근본 해결은 `_intraday_to_base` (`src/data/intraday.py:179, 363`) 에 prev_close 와 같은 fetch_quote 보강 적용. 단 dashboard worker 의 tick 주기 (3초) 에서 fetch_quote 추가는 rate limit 영향 — 비용 검토 후 진행.
...
- [ ] **회전율 매칭 layer (layer3_high_turnover)** — 2026-05-24 master 에 상장주식수(shares) 적재 완료로 unblock. 과거 일별 시총 = 과거 종가 × shares 로 historical 회전율 산출 가능. gap_stats 에 layer3_high_turnover 추가 (기존 layer3_high_vol = 거래량비율 매칭과 별도 축) — 별도 round.
...
- [x] **fetch_quote / fetch_quotes_bulk 도 hts_avls 사용 (2026-05-24)** — `fetch_quote` 가 `hts_avls`(시가총액, 억원) + `tr_pbmn_tnrt`(거래대금회전율) 를 직접 파싱해 market_cap/turnover 채움. hts_avls 비면 `infer_market_cap_eok` 로 회전율 역산. `fetch_quotes_bulk` 는 master 시총 유효할 때만 덮어쓰고 0(파싱 깨짐)이면 fetch_quote 값 보존. `_enrich_candidates_with_quote` 의 `_MISSING_TARGETS` 에 market_cap/turnover 추가.
- [x] **종목 마스터 market_cap 적재 (2026-05-24)** — KIS mst part2 char[172:181] 슬라이스가 모두 0 으로 파싱되는 결함. pykrx/KRX public 은 이 데이터셋 날짜(2026)에 빈 응답 + 프로젝트가 KIS 기반이라 외부 시총 끌어오면 가격 불일치 → **누적 거래대금 순위 스냅샷에서 `시총=거래대금/(회전율/100)` 역산 backfill** 채택 (`master.backfill_market_cap_from_snapshots`, `infer_market_cap_eok` 과 동일 정의라 표시 회전율과 자기일관). 종목별 최신 스냅샷 채택, 상장주식수(=시총/현재가)도 함께 저장 → 과거 일별 시총=과거종가×상장주식수 로 historical 회전율 layer 가능. `update_master.py` 에 wiring (daily 16:30). 현재 적재: 거래대금 top50 등장 120/2731 종목 (저유동 종배 무관 종목은 0 유지). `STOCK_MASTER_COLUMNS` 에 `shares` 추가. tests: backfill 3건 신규 (`test_master.py`). 964 pass. 잔여: 전 종목 커버리지(저유동 포함) 필요 시 KIS inquire-price `hts_avls`/`lstn_stcn` 일괄 backfill (2731콜, 별도 round).
- [x] **결정 레포트 일봉 "0 → X" prev_close 버그 (2026-05-24)** — 05-21 정정 때 넣은 daily 종가 fallback 이 `_enrich_candidates_with_quote` 의 `from src.data.daily import read_daily_ohlcv` (실제 위치는 `src.data.storage`) 오타로 ImportError → except 에 삼켜져 영영 실행 안 됨 (정정이 미배포 상태). import 모듈 수정. 표시 자체는 사용자 요청으로 현재가+상승률로 변경되어 prev_close 미표시이나, prev_close 는 close_position(Layer 3 매칭) 계산에 여전히 쓰이므로 fix 유지.
- [x] **종배 빌드 batch — 그룹/양봉/forward/막판/NXT (2026-05-25, "쭉 이어서 다 해줘")** — ①텔레그램 종배 그룹 라우팅(`TELEGRAM_EOD_CHAT_ID`, config+dispatcher; 종배 레포트만 그룹, 단타 M6/명령/에러는 DM). ②양봉/장대양봉 카운트 표시(`gap_stats.candle_count_aux`, scheduler+pipeline+decision, 표시만). ③forward 로깅(`src/overnight/forward_log.py` append_outcomes/load_outcomes, scheduler 16:40 cron — 14:50 후보+다음날 갭, 수급/체결강도 미래 factor_edge용). ④막판 진입 점검(`src/overnight/eod_entry.py` build_eod_entry_context/format, scheduler 15:00/10/20 cron, dispatcher.send_eod_entry — VP/점상한가/고점대비 표시, hard rule X). ⑤NXT 가능/불가/추정 표시(`src/overnight/nxt.py`, `data/meta/nxt_tradable.txt` pluggable). `fetch_quote` 에 open(stck_oprc) 추가. tests +24 (test_candle_aux/forward_log/eod_entry/nxt + notify 라우팅 3), **1004 pass**. 후속: 막판 점검에 **분봉 가속(compute_accel_ratio)+봉형태(is_weak_candle) 연동 완료** + forward self-heal 백필 + .env.example EOD 항목 (1005 pass). **잔여 v1: NXT 목록 크롤러(nextrade.co.kr) + KIS NXT 시세 API(레포 grep) / forward 누적 후 factor_edge 재실행.**
- [x] **Eod.Exit v2 — 라이브 청산 지원 (2026-05-25)** — 청산 타이밍(~9%p) ≫ 선별(+0.7%)=13배 (`backtest_recent_kelly.py`). 청산 타이밍 backtest 불가(분봉 부재) → 새 임계값 X, **검증된 ≤1/1-6/≥6% 룰을 현재가에 라이브 재평가 + 고점대비 되돌림 표시**. `evaluate_overnight_exit_live(prev_close, current, intraday_high, open_price=None)` → `OvernightExitContext` + `format_overnight_exit_line` (`src/overnight/exit.py`, `_classify_gap` 단일출처 리팩터). `fetch_quote` 에 시가(`open`=stck_oprc) 추가. scheduler `_send_jongbae_open_exit_recommendation` 라이브 엔진 전환 + cron 09:01→**09:01/10/20/30 다회 체크인**. 시초 +7%(분할)→현재 +2% fade 시 '전량 익절' 자동 전환. 자동 주문 X. `test_jongbae_exit.py` 7 신규, 980 pass. **다음(②후속/③): NXT 프리장(08:00~) 청산 분산 — KIS NXT API 검증 후 / 막판 진입 게이트.**
- [x] **Eod.Sizing v2 — 거래대금순위 버킷 Kelly + top3 순위/비중 (2026-05-25)** — factor_edge backtest(250일+3개월): **거래대금순위만 robust**(top10 갭상 1년 55%/3개월 64%, 단조), 끼/신고가/신고가부근/시총/종가위치/거래량은 노이즈(창 사이 뒤집힘, 다중비교). `src/overnight/sizing_bucket.py` 신설 — 거래대금순위 버킷(1~10/11~25/26~50위) rolling p/W/L → `kelly_fraction` 재사용. scheduler `_send_decision_report` + `pipeline.run_pipeline` 양쪽에 버킷 사이징 + 거래대금순위 정렬 + top3 플래그 wiring. decision 레포트: 후보 헤더 `⭐[N위]`, 후보별 `비중(Kelly) X%(계좌)`, `[오늘의 top3 — 비중]` 블록(절대 + top3내 상대 강약 + 투입/현금). per-종목 3방식(균등/Kelly/Sharpe)은 참고 강등. 검증 스크립트 `scripts/backtest_{universe_compare,factor_edge,factor_edge2,top3_selection,recent_kelly}.py` (결과 `data/backtest/*.md`). **★ 청산 타이밍 envelope(~9%p) ≫ 선별 엣지(+0.7%) = 13배 → 다음 빌드: 시초/NXT 청산 지원 (최고 레버리지).** `tests/test_sizing_bucket.py` 6 신규, 973 pass. memory: [[project-eod-factor-edge]].
- [x] **결정 레포트 카드 3거래일 추이 + 표시 정정 (2026-05-24)** — 사용자 7개 요청. `src/overnight/candidate_trends.py` 신규 (거래대금/회전율 값 추이 = daily ohlcv, 순위 변동 = 14:50 스냅샷 top50, 수급 추이 = investor_daily 누적). `_candidate_block` 재작성: 현재가+상승률 / "현재가 대비" / 괄호 중복단어 제거 / 시총 별도 줄 / "표시만"·"자체 누적 시작" 문구 제거 / 3일 추이 라인. scheduler+pipeline 양쪽 `attach_candidate_trends` 호출. tests 13 신규 (`test_candidate_trends.py` 8 + `test_report.py` 5).
- [x] **dashboard_worker 14건 tz 실패 (2026-05-24)** — `tests/test_dashboard_worker.py` 14건이 `triggers.py` `now - holding.entry_time` 에서 `can't subtract offset-naive and offset-aware datetimes` 로 실패. 근본 원인 2개: ①견고성 — `evaluate_triggers` 의 시각 빼기(entry_time / vol_drain_since / vi_triggered_at) 가 naive/aware 혼재 미처리. ②테스트 격리 — conftest 에 DATA_DIR 격리 없어 `load_holdings()` 가 실제 `data/state/holdings.json` (영속화 entry_time +09:00 aware) 을 읽어 naive now 와 충돌 + 무관 보유종목이 카드로 surface. fix: ①`_elapsed_seconds(then, now)` 헬퍼 — aware 는 KST 로 변환 후 wall-clock 통일 (`maybe_reset_holdings` 의 "naive/aware 모두 허용" 계약을 시각 연산에도 일관 적용), 3개 사이트 적용. ②`test_dashboard_worker.py` autouse fixture 로 `load_holdings` → {} 격리 (보유 테스트는 로컬 patch override). tests: 견고성 회귀 2건(`test_A4_naive_now_with_aware_entry_time` / `_aware_now_with_naive_entry_time`) 신규. 961 pass.
- [x] **16:00 사후 레포트 None sizing 크래시 (2026-05-24)** — 텔레그램 검증 발송 중 발견. `build_afterhours_report` 가 reload 된 후보(`load_decision_candidates`, scheduler:407)의 `sizing_stats.p` 가 None 일 때 `TypeError: NoneType * int` 로 죽음. 원인: Layer 사례 0건 종목(round 41 유니버스 확장으로 들어온 대형주, 예: 삼성전기 — 1년 ret≥20% 0건)은 sizing p/avg=NaN → save_decision_candidates 가 NaN→null 직렬화 → reload 시 None. `p == p` NaN 가드가 `None == None`(True) 을 못 걸러 `None*100` 크래시. fix: `formatting.is_num(x)` 공통 가드(int/float & not NaN & not None & not bool) 도입 → `fmt_pct` / `fmt_layer_stats` / `fmt_sizing_table` / `afterhours` 가드 일괄 교체. 같은 reload 경로인 `build_decision_report` _sizing_block 도 자동 견고화. 실제 05-22 저장 후보로 사후+결정 레포트 재현 후 fix 검증. tests: afterhours None / decision None / is_num·fmt None 3건 신규. 967 pass.
