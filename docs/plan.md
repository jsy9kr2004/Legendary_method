...
- [x] **결정 레포트 수동 재실행 CLI (2026-05-19)** — 사용자 보고: "오늘 기준 자료로 고친 건 못 받아?". 14:50 cron 이 이미 돌고 fix 가 들어가도 다음 영업일 14:50 까지 대기 → fix 검증 사이클 너무 김. 추가: `src/rerun_decision.py` 신규 CLI — 저장된 스냅샷 (`data/snapshots/YYYY-MM-DD/14_50.parquet`) 로드 + `scheduler._send_decision_report` 직접 호출 → 호출 시점 KIS 응답으로 OHLCV 보강 / 시장 국면 / 14:50 시그널 fetch + 텔레그램 재발송. `--date` / `--snapshot` 옵션. preview 만 원하면 `DRY_RUN=1` 환경변수. `./go decision-rerun` 추가. ⚠ 주의: OHLCV / 시장 국면 / 시그널은 14:50 당시 값이 아니라 **현 시각 (after-hours) KIS 응답** — 시초 청산 시점에는 차이 미미하나 분석용으로만. 정시 cron 의 14:50_decision.md 저장본 덮어씀.
...
- [ ] **R4 v2 결정 레포트 룰 코드 적용 (round 41, (e) 완료 / (a)(c)(d)(f) 잔여)** — 사용자(Zeta) 5/19 사후 검증 후 새 룰 확정: `(a) 거래대금 50위 단일종목 + (b) 일봉 상승 + (c) 종가 고가-10% 이내 + (d) 52주 신고가 + (e) 10% ≤ ret ≤ 27% + (f) Layer 표본 ≥5`. 문서는 `docs/jongbae-strategy.md` R4 v2 + 정정 이력 round 41 에 박음. 코드 적용 진행 상황: ①**(e) 완료 (2026-05-19)** — `src/jongbae/candidates.py` `MIN_DAILY_RETURN`: 20.0 → 10.0, `MAX_DAILY_RETURN=27.0` 신규. 상한 컷이 우선 → 상한가(+30%≈) / 자리잡힘(+28~29.5%) 자동 제외. `STUCK_AT_28_RANGE` / `DEAD_PULL_THRESHOLD` 상수 삭제 (모두 컷 범위 밖). `PRIORITY_LIMIT_UP` 은 backward-compat 위해 상수만 유지, `classify_priority` 가 더는 반환 X. 사용자 보고 회귀 fix (진원생명과학 011000 +29.97%). 테스트 7건 갱신/신규 — `test_classify_excluded_limit_up_above_27` / `_normal_between_10_and_20` / `_excluded_below_10` / `_excluded_above_27_stuck_at_28` / `test_extract_candidates_excludes_user_reported_regression`. ②(b) 명시 룰 미적용 — 현재 daily_return ≥ 10% 만 검사. ③(a) 거래대금 50위 단일종목 — R3 leading_theme 의존 우회 → `extract_candidates_v2(snapshot_df, daily_ohlcv)` 신규 또는 기존 함수 재설계. ④(c) 종가 고가-10% 이내 — `intraday_high` 와 `price` 로 `(high - close) / high ≤ 10%` 검사. ⑤(d) 52주 신고가 — `daily_ohlcv` lookback. ⑥(f) Layer 표본 ≥5 — `has_enough_samples` 이미 적용. ⑦`src/jongbae/historical.py` `historical_ret10_gap_stats` 추가 — 카드/레포트 보조 표시. ⑧`_candidate_block` 에 historical 보조 라인 추가. ⑨레포트 본문 "v2 룰 적용" 푸터. ⑩누적 backtest — 1~3개월 v2 결과 `data/decisions/` 적재 후 갭상 확률 재측정.
