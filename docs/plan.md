...
- [x] **STRONG 푸시 미수신 + 카드/PWA 30초 지연 긴급 픽스 (2026-05-29)** ★ — 사용자 보고. 근본 원인 2개: ①`worker.dashboard_tick` 의 `push_mr_event(..., float(mr_score), ...)` 가 v11 리팩터링(score→sc_buy/sc_sell) 때 정의 안 된 로컬 `mr_score` 참조 → sigB/sigS 발화 순간 NameError 로 루프 중단 → `_maybe_push_mr_strong_alert` 가 영영 호출 안 됨 (STRONG 푸시 한 번도 못 받은 진짜 이유). fix=`float(sc_buy/sc_sell)`. ②카드 edit 동기 호출 + 매 호출 새 `httpx.Client()` + 429 tenacity 지수백오프가 tick 쓰레드 블로킹 → 종목 직렬 누적 = tick 수십 초 → PWA `last_payloads`·STRONG 푸시 동반 지연. fix=전용 송출 데몬 쓰레드 `src/dashboard/telegram_sender.py` (`TelegramCardSender`: 영속 connection·latest-wins·dedup·429 Retry-After 격리). `dashboard_tick`/`_maybe_push_mr_strong_alert`/`cleanup_messages` 옵셔널 `card_sender` 배선(없으면 동기 = 하위호환), `scheduler.py` lazy 싱글톤. tests: sender 8 + worker 2(회귀+라우팅) + strong_alert 1 = 11 신규. 상세: `docs/scalping-redesign-2026-05-27.md` §11.
