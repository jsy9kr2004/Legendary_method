...
- [ ] **종목 마스터 market_cap 적재** — KIS mst part2 char[172:181] 슬라이스가 모두 0 으로 파싱됨. mst 포맷 검증 또는 pykrx/KIS inquire-price 로 별도 backfill 필요. **결정 레포트 시총 표시는 2026-05-24 `infer_market_cap_eok`(거래대금/회전율 역산) 으로 우회 완료** — 하지만 historical 분석/회전율 매칭 layer (과거 일별 시총 필요) 엔 여전히 정식 적재 필요.
