...
| Round | 잘못 알았던 것 | 정정 |
|---|---|---|
| 2026-05-22 (주도섹터 universe 교집합) | Theme 주도섹터 분모를 거래대금 top 50 단일 기준으로만 잡음 — 대형주 편향으로 단타 자금 실유입 종목 희석. 사용자(Zeta) 명시: "거래대금 50위 & 회전율(거래대금/시총) 50위 교집합에서 주도섹터 원래 룰대로(갯수 많은거)". 단타/종배 양쪽 공통. | `src/common/theme.py::_sector_universe` 신설 — 분모 = 거래대금 top_n ∩ 회전율 top_n. 단타 dashboard 경로 `score_leading_sectors`(v1 z-score) + 종배 경로 `identify_leading_themes`(v0 count) 둘 다 적용. 교집합 위 주도섹터 채택 룰 불변. turnover 없으면 거래대금 top_n fallback. 회귀 4건 신규. 952 pass. 상세는 `docs/eod-strategy.md` 정정 이력 동일 행. |
