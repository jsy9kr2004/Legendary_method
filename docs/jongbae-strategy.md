...
(보유 모드, R15 — `/buy` 로 진입)
- 매수가, 현재 손익 %
- 손절선 (-2%, 진입 봉 저점, 5분 이평) — **알림만, 실주문 X** (정정 2026-05-21: A1 -1.5% → -2% 사용자 룰 통일)
...
| 41 후속 2 | round 41 후속 (d)(f) soft 정정 후 사용자 추가 보고 (2026-05-19): 후보가 여전히 주도섹터 종목만 나옴 + 그 종목들이 top 50 거래대금에도 안 들어옴 + 50위 안 ret=20 엑스게이트는 후보로 선정 X. (a) 주도섹터 우회 코드는 검증 완료 (`leading_theme_codes=None`) — 다른 원인. 사용자 명시 요청: "ret 하한을 10에서 5로 낮춰줘". 추가 원인 가설: KIS volume-rank API `data_rank` 가 HTS 거래대금 순위와 다를 가능성 (KIS 가 ETF/펀드 포함 절대 순위, HTS 는 별도 필터링) + master_df 누락으로 엑스게이트 같은 신생/특수 종목 필터링 가능성. **본 round 는 사용자 명시 요청 (5% 하한) + 진단 인프라만**. KIS rank vs HTS 순위 mismatch 본격 조사는 사용자가 진단 로그로 실측 후 별도 round. | round 41 후속 2: ①`src/jongbae/candidates.MIN_DAILY_RETURN` 10.0 → 5.0. 모듈 docstring 정정 이력 박음. ②`src/scheduler._send_decision_report` 진단 로깅 신규 — snapshot 의 KIS rank 범위 + top 10 (rank/code/name/ret/거래대금) + 제외된 종목들의 사유 (rank/code/name/ret/exclusion_reason). 사용자가 `./go logs` 또는 `./go decision-rerun` 후 로그로 (1) snapshot 에 어떤 종목이 어떤 rank 로 들어왔는지 (2) 엑스게이트가 snapshot 에 있었는지 + 있었으면 어디서 떨어졌는지 (3) 후보 2종목이 어떤 KIS rank 로 들어왔는지 즉시 확인 가능. ③테스트: `test_classify_excluded_below_10` → `_excluded_below_5` 이름·임계 갱신, `_normal_between_10_and_20` → `_normal_between_5_and_20` + `_normal_at_lower_bound_5pct` 신규. 897 pass. **다음 round 예상 작업** (사용자 진단 로그 결과 확인 후): (A) KIS data_rank vs HTS 순위 mismatch 면 — fetch_volume_rank 결과를 trading_value desc 로 재정렬하고 rank 재부여하는 옵션 / 또는 다른 KIS endpoint (`order-book` 등) 평가. (B) master_df 필터 누락이면 — `update_master` cron 빈도 / 특수 종목 (신생 상장 / 거래정지 후 재개 등) 처리 정책. (C) 둘 다 아니면 — KIS volume-rank 가 한 번 호출에 30개만 반환하는 한계 (`top_n=50` 효과 X) 라면 페이지네이션 또는 다른 endpoint. |
