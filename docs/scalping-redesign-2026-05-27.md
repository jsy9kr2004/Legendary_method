...
| **2026-05-29 (긴급 버그픽스)** | **STRONG 푸시 미수신 근본 원인 = `mr_score` NameError + tick 블로킹** | 사용자 보고: "텔레그램 카드·PWA 둘 다 30초마다 갱신 + STRONG 푸시 한 번도 못 받음". 진단: ①`worker.dashboard_tick` 의 `push_mr_event(now, kind, float(mr_score), ...)` 가 v11 리팩터링(score→sc_buy/sc_sell 분리) 때 정의 안 된 로컬 `mr_score` 참조 — **sigB/sigS 발화 순간(=STRONG 푸시가 떠야 할 시점) NameError 로 종목 루프 중단** → 그 아래 `_maybe_push_mr_strong_alert` 가 영영 호출 안 됨. fix: `float(sc_buy)`(단저)/`float(sc_sell)`(단고). ②카드 edit 이 tick 안에서 동기 호출 + 매번 새 `httpx.Client()`(핸드셰이크 반복) + 429 를 tenacity 가 tick 쓰레드에서 지수백오프 → 종목 6~10 개 직렬 누적 = tick 수십 초 → 같은 tick 의 PWA `last_payloads`·STRONG 푸시까지 동반 지연. fix: 카드 송출 전용 데몬 쓰레드 `src/dashboard/telegram_sender.py` (`TelegramCardSender`) — 영속 connection·latest-wins·dedup·429 `Retry-After` 격리. `dashboard_tick`/`_maybe_push_mr_strong_alert`/`cleanup_messages` 에 옵셔널 `card_sender` 배선 (없으면 기존 동기 경로 = 하위호환). `scheduler.py` 에서 lazy 싱글톤 생성·종료 시 stop. tests: `test_telegram_card_sender.py` 8 신규 + `test_dashboard_worker.py` 2 신규(NameError 회귀 + card_sender 라우팅) + `test_mr_strong_alert.py` 1 신규(위임). |
