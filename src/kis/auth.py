...
발급(`/oauth2/tokenP`) → 파일 캐시(`{DATA_DIR}/meta/kis_token.json`) →
만료 5분 전 자동 재발급.

api_mode (real/mock) 별 토큰이 다르므로 캐시도 분리한다.
"""
...
_TOKEN_FILENAME = "kis_token.json"
...
def _token_path(settings: Settings) -> Path:
    return settings.data_dir / "meta" / _TOKEN_FILENAME


def _load_cached(settings: Settings) -> Token | None:
...
def _save_cache(token: Token, settings: Settings) -> None:
    path = _token_path(settings)
...
def _request_new_token(settings: Settings) -> Token:
    ...
    if "access_token" not in payload:
        raise RuntimeError(
            f"KIS 토큰 발급 실패 (mode={settings.kis_api_mode}): {payload}"
        )
...
def get_token(settings: Settings, force_refresh: bool = False) -> Token:
    ...
    logger.info(f"KIS 토큰 신규 발급 (mode={settings.kis_api_mode})")
    token = _request_new_token(settings)
    _save_cache(token, settings)
    return token
